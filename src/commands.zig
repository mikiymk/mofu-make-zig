...
pub export fn fatal_error_signal(arg_sig: c_int) void {
    ...
    if ((((sig == 15) or (sig == 2)) or (sig == 1)) or (sig == 3)) {
        var c: ?*struct_child = undefined;
        {
            c = children;
            while (c != null) : (c = c.*.next) if ((@as(c_int, @bitCast(c.*.remote)) != 0) and (c.*.pid > 0)) {
    ...
    } else while (job_slots_used > uint) {
        reap_children(1, 1);
    }
...
pub export fn execute_file_commands(arg_file_1: ?*struct_file) void {
    ...
    if ((@as(c_int, @bitCast(file_1.*.loaded)) != 0) and (unload_file(file_1.*.name) == 0)) {
        file_1.*.loaded = 0;
        file_1.*.unloaded = 1;
    }
    new_job(file_1);
...
pub export fn delete_child_targets(arg_child_1: ?*struct_child) void {
    ...
    if ((@as(c_int, @bitCast(child_1.*.deleted)) != 0) or (child_1.*.pid < 0)) return;
...
pub fn delete_target(arg_file_1: ?*struct_file, arg_on_behalf_of: [*c]const u8) callconv(.C) void {
    ...
    if (ar_name(file_1.*.name) != 0) {
        var file_date: time_t = if (file_1.*.last_mtime == @as(uintmax_t, 1)) @as(time_t, @bitCast(@as(c_long, -1))) else @as(time_t, @bitCast((file_1.*.last_mtime -% @as(uintmax_t, @bitCast(@as(c_long, 2 + 1)))) >> @intCast(if (true) 30 else 0)));
        if (ar_member_date(file_1.*.name) != file_date) {
            if (on_behalf_of != null) {
                c_error_fn_replaced_for_reserved_keyword(@as([*c]floc, @ptrFromInt(0)), strlen(on_behalf_of) +% strlen(file_1.*.name), gettext("*** [%s] Archive member '%s' may be bogus; not deleted"), on_behalf_of, file_1.*.name);
            } else {
                c_error_fn_replaced_for_reserved_keyword(@as([*c]floc, @ptrFromInt(0)), strlen(file_1.*.name), gettext("*** Archive member '%s' may be bogus; not deleted"), file_1.*.name);
            }
        }
        return;
    }
    ...
    if (((e == 0) and ((st.st_mode & @as(__mode_t, 61440)) == @as(__mode_t, 32768))) and (file_timestamp_cons(file_1.*.name, st.st_mtim.tv_sec, st.st_mtim.tv_nsec) != file_1.*.last_mtime)) {
        if (on_behalf_of != null) {
            c_error_fn_replaced_for_reserved_keyword(@as([*c]floc, @ptrFromInt(0)), strlen(on_behalf_of) +% strlen(file_1.*.name), gettext("*** [%s] Deleting file '%s'"), on_behalf_of, file_1.*.name);
        } else {
            c_error_fn_replaced_for_reserved_keyword(@as([*c]floc, @ptrFromInt(0)), strlen(file_1.*.name), gettext("*** Deleting file '%s'"), file_1.*.name);
        }
...
pub inline fn DEFINE_VARIABLE(name: anytype, len: anytype, value: anytype) anyopaque {
    return @import("std").zig.c_translation.cast(anyopaque, define_variable_for_file(name, len, value, o_automatic, 0, file));
}
