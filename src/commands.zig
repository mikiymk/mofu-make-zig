...
pub export fn execute_file_commands(arg_file_1: [*c]struct_file) void {
    ...
    _ = define_variable_in_set("%", @as(usize, @bitCast(@as(c_long, @as(c_int, 1)))), percent, @as(c_uint, @bitCast(o_automatic)), @as(c_int, 0), file_1.*.variables.*.set, @as([*c]floc, @ptrFromInt(@as(c_int, 0))));
    {
        ...
        qmark_len = plus_len +% @as(usize, @bitCast(@as(c_long, @as(c_int, 1))));
        {
            ...
            while (d != null) : (d = d.*.next) if ((!(d.*.ignore_mtime != 0) and !(d.*.need_2nd_expansion != 0)) and !(d.*.ignore_automatic_vars != 0)) {
                ...
                if (!((@as(c_int, @bitCast(d.*.changed)) != 0) or (always_make_flag != 0))) {
        ...
        hash_init(&dep_hash, @as(c_ulong, @bitCast(@as(c_long, @as(c_int, 500)))), &dep_hash_1, &dep_hash_2, &dep_hash_cmp);
        {
            d = file_1.*.deps;
            while (d != null) : (d = d.*.next) {
                if ((@as(c_int, @bitCast(d.*.need_2nd_expansion)) != 0) or (@as(c_int, @bitCast(d.*.ignore_automatic_vars)) != 0)) continue;
                ...
                if ((slot.* == null) or (slot.* == hash_deleted_item)) {
                    ...
                    var hd: ?*struct_dep = @as(?*struct_dep, @ptrCast(slot.*));
                    _ = &hd;
                    if (@as(c_int, @bitCast(d.*.ignore_mtime)) != @as(c_int, @bitCast(hd.*.ignore_mtime))) {
                        d.*.ignore_mtime = blk: {
        ...
        }
        {
            ...
            while (d != null) : (d = d.*.next) {
                ...
                _ = &c;
                if (((@as(c_int, @bitCast(d.*.need_2nd_expansion)) != 0) or (@as(c_int, @bitCast(d.*.ignore_automatic_vars)) != 0)) or (hash_find_item(&dep_hash, @as(?*const anyopaque, @ptrCast(d))) != @as(?*anyopaque, @ptrCast(d)))) continue;
...
pub fn delete_target(arg_file_1: [*c]struct_file, arg_on_behalf_of: [*c]const u8) callconv(.C) void {
    var file_1 = arg_file_1;
    _ = &file_1;
...
pub inline fn DEFINE_VARIABLE(name: anytype, len: anytype, value: anytype) anyopaque {
    ...
    _ = &len;
    _ = &value;
    return @import("std").zig.c_translation.cast(anyopaque, define_variable_for_file(name, len, value, o_automatic, @as(c_int, 0), file));
}
