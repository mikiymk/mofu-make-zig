...
pub export fn strcache_init() void {
    hash_init(&strings, ulong, &str_hash_1, &str_hash_2, &str_hash_cmp);
}
pub const sc_buflen_t = c_ushort;
pub const struct_strcache = extern struct {
...
pub export fn strcache_print_stats(arg_prefix: [*c]const u8) void {
    ...
    var minfree: c_ulong = (ulong -% (ulong *% @sizeOf(usize))) -% @offsetOf(struct_strcache, "buffer");
    if (!(strcache != null)) {
    ...
    _ = 0;
    _ = printf(gettext("\n%s strcache buffers: %lu (%lu) / strings = %lu / storage = %lu B / avg = %lu B\n"), prefix, numbuffs +% ulong, fullbuffs, total_strings, total_size, total_size / total_strings);
    _ = printf(gettext("%s current buf: size = %hu B / used = %hu B / count = %hu / avg = %u B\n"), prefix, @as(c_int, @bitCast(@as(c_uint, @as(sc_buflen_t, @bitCast(@as(c_ushort, @truncate((ulong -% (ulong *% @sizeOf(usize))) -% @offsetOf(struct_strcache, "buffer")))))))), @as(c_int, @bitCast(@as(c_uint, strcache.*.end))), @as(c_int, @bitCast(@as(c_uint, strcache.*.count))), @as(c_uint, @bitCast(@divTrunc(@as(c_int, @bitCast(@as(c_uint, strcache.*.end))), @as(c_int, @bitCast(@as(c_uint, strcache.*.count)))))));
    if (numbuffs != 0) {
...
pub fn add_string(arg_str: [*c]const u8, arg_len: sc_buflen_t) callconv(.C) [*c]const u8 {
    ...
    if (@as(c_ulong, @bitCast(@as(c_ulong, sz))) > ((ulong -% (ulong *% @sizeOf(usize))) -% @offsetOf(struct_strcache, "buffer"))) {
    ...
    if (sp == @as([*c]struct_strcache, @ptrCast(@alignCast(@as(?*anyopaque, @ptrFromInt(0)))))) {
        sp = new_cache(&strcache, @as(sc_buflen_t, @bitCast(@as(c_ushort, @truncate((ulong -% (ulong *% @sizeOf(usize))) -% @offsetOf(struct_strcache, "buffer"))))));
        spp = &strcache;
    }
    res = copy_string(sp, str, len);
    if ((total_strings > ulong) and (@as(c_ulong, @bitCast(@as(c_ulong, sp.*.bytesfree))) < ((total_size / total_strings) +% ulong))) {
        spp.* = sp.*.next;
        sp.*.next = fullcache;
        fullcache = sp;
    }
...
pub fn add_hash(arg_str: [*c]const u8, arg_len: usize) callconv(.C) [*c]const u8 {
    ...
    if (len > @as(usize, @bitCast(@as(c_long, ((32767 * 2) + 1) - 1)))) return add_hugestring(str, len);
...
pub const CACHE_BUFFER_BASE = 8192;
pub inline fn CACHE_BUFFER_ALLOC(_s: anytype) @TypeOf(_s - (2 * @import("std").zig.c_translation.sizeof(usize))) {
    return _s - (2 * @import("std").zig.c_translation.sizeof(usize));
}
...
pub const hugestring = struct_hugestring;
