...
pub export fn split_prereqs(arg_p: [*c]u8) ?*struct_dep {
    var p = arg_p;
    var new: ?*struct_dep = @as(?*struct_dep, @ptrCast(parse_file_seq(&p, @sizeOf(struct_dep), 256, null, 64)));
    if (p.* != 0) {
        var ood: ?*struct_dep = undefined;
        p += 1;
        ood = @as(?*struct_dep, @ptrCast(parse_file_seq(&p, @sizeOf(struct_dep), 1, null, 64)));
        if (!(new != null)) {
            new = ood;
        } else {
            ...
            {
                dp = new;
                while (dp.*.next != @as(?*struct_dep, @ptrCast(@as(?*anyopaque, @ptrFromInt(0))))) : (dp = dp.*.next) {}
            }
            dp.*.next = ood;
        }
        while (ood != @as(?*struct_dep, @ptrCast(@as(?*anyopaque, @ptrFromInt(0))))) : (ood = ood.*.next) {
            ood.*.ignore_mtime = 1;
        }
    }
    return new;
...
pub export fn expand_deps(arg_f: ?*struct_file) void {
    ...
    while (d != null) {
        ...
        if (d.*.staticpattern != 0) {
            ...
            if (nperc != 0) {
                var slen: usize = (strlen(d.*.name) +% nperc) +% ulong;
...
pub export fn remove_intermediates(arg_sig: c_int) void {
    ...
    while (file_slot < file_end) : (file_slot += 1) if (!((file_slot.* == null) or (@as(?*anyopaque, @ptrCast(file_slot.*)) == hash_deleted_item))) {
        ...
        if (((((@as(c_int, @bitCast(f.*.intermediate)) != 0) and ((@as(c_int, @bitCast(f.*.dontcare)) != 0) or !(f.*.precious != 0))) and !(f.*.secondary != 0)) and !(f.*.notintermediate != 0)) and !(f.*.cmd_target != 0)) {
            ...
            if (!(f.*.dontcare != 0)) {
                if (sig != 0) {
                    c_error_fn_replaced_for_reserved_keyword(@as([*c]floc, @ptrFromInt(0)), strlen(f.*.name), gettext("*** Deleting intermediate file '%s'"), f.*.name);
                } else {
...
pub export fn snap_deps() void {
    ...
    if ((f != null) and (@as(c_int, @bitCast(f.*.is_target)) != 0)) {
        ...
        if (!(f.*.deps != null)) {
            ...
            d = f.*.deps;
            while (d != @as(?*struct_dep, @ptrCast(@as(?*anyopaque, @ptrFromInt(0))))) : (d = d.*.next) {
                f2 = d.*.file;
                while (f2 != @as(?*struct_file, @ptrCast(@as(?*anyopaque, @ptrFromInt(0))))) : (f2 = f2.*.prev) if (f2.*.deps != null) {
                    d2 = f2.*.deps.*.next;
                    while (d2 != @as(?*struct_dep, @ptrCast(@as(?*anyopaque, @ptrFromInt(0))))) : (d2 = d2.*.next) {
                        d2.*.wait_here = 1;
                    }
                };
            }
    ...
    {
        var prereqs: ?*struct_dep = expand_extra_prereqs(lookup_variable(".EXTRA_PREREQS", @sizeOf([15]u8) -% ulong));
...
pub export fn set_command_state(arg_file_1: ?*struct_file, arg_state: enum_cmd_state_36) void {
...
pub export fn init_hash_files() void {
    hash_init(&files, ulong, &file_hash_1, &file_hash_2, &file_hash_cmp);
}
...
pub export fn build_target_list(arg_value: [*c]u8) [*c]u8 {
    ...
    if (files.ht_fill != last_targ_count.static) {
        var max: usize = ((strlen(value) / ulong) +% ulong) *% ulong;
        ...
        while (fp < end) : (fp += 1) if (!((fp.* == null) or (@as(?*anyopaque, @ptrCast(fp.*)) == hash_deleted_item)) and (@as(c_int, @bitCast(fp.*.*.is_target)) != 0)) {
            ...
            if (len > max) {
                ...
                max +%= (((l +% @as(usize, 1)) / @as(usize, 500)) +% @as(usize, 1)) *% @as(usize, 500);
                value = @as([*c]u8, @ptrCast(@alignCast(xrealloc(@as(?*anyopaque, @ptrCast(value)), max))));
                p = &value[off];
            }
        ...
        (p - @as(usize, @bitCast(@as(isize, 1)))).* = '\x00';
...
pub export fn file_timestamp_cons(arg_fname: [*c]const u8, arg_stamp: time_t, arg_ns: c_long) uintmax_t {
    ...
    var stamp = arg_stamp;
    var ns = arg_ns;
    var offset: c_int = @as(c_int, @bitCast(@as(c_int, @truncate(@as(c_long, @bitCast(@as(c_long, 2 + 1))) + (if (true) ns else long)))));
    var s: uintmax_t = @as(uintmax_t, @bitCast(stamp));
    var product: uintmax_t = s << @intCast(if (true) 30 else 0);
    var ts: uintmax_t = product +% @as(uintmax_t, @bitCast(@as(c_long, offset)));
    if (!(((s <= (((((((((~@as(uintmax_t, 0) -% (if (!(@as(uintmax_t, @bitCast(@as(c_long, -1))) <= @as(uintmax_t, 0))) @as(uintmax_t, 0) else ~@as(uintmax_t, 0) << @intCast((@sizeOf(uintmax_t) *% ulong) -% ulong))) -% @as(uintmax_t, @bitCast(@as(c_long, 2 + 1)))) >> @intCast(if (true) 30 else 0)) << @intCast(if (true) 30 else 0)) +% @as(uintmax_t, @bitCast(@as(c_long, 2 + 1)))) +% @as(uintmax_t, @bitCast(@as(c_long, if (true) 1000000000 else 1)))) -% @as(uintmax_t, 1)) -% @as(uintmax_t, @bitCast(@as(c_long, 2 + 1)))) >> @intCast(if (true) 30 else 0))) and (product <= ts)) and (ts <= (((((((~@as(uintmax_t, 0) -% (if (!(@as(uintmax_t, @bitCast(@as(c_long, -1))) <= @as(uintmax_t, 0))) @as(uintmax_t, 0) else ~@as(uintmax_t, 0) << @intCast((@sizeOf(uintmax_t) *% ulong) -% ulong))) -% @as(uintmax_t, @bitCast(@as(c_long, 2 + 1)))) >> @intCast(if (true) 30 else 0)) << @intCast(if (true) 30 else 0)) +% @as(uintmax_t, @bitCast(@as(c_long, 2 + 1)))) +% @as(uintmax_t, @bitCast(@as(c_long, if (true) 1000000000 else 1)))) -% @as(uintmax_t, 1))))) {
        ...
        c_error_fn_replaced_for_reserved_keyword(@as([*c]floc, @ptrFromInt(0)), strlen(f) +% strlen(@as([*c]u8, @ptrCast(@alignCast(&buf)))), gettext("%s: Timestamp out of range; substituting %s"), f, @as([*c]u8, @ptrCast(@alignCast(&buf))));
    }
    return ts;
}
...
pub export fn file_timestamp_sprintf(arg_p: [*c]u8, arg_ts: uintmax_t) void {
    var p = arg_p;
    var ts = arg_ts;
    var t: time_t = @as(time_t, @bitCast((ts -% @as(uintmax_t, @bitCast(@as(c_long, 2 + 1)))) >> @intCast(if (true) 30 else 0)));
    var tm_1: [*c]struct_tm = localtime(&t);
    if (tm_1 != null) {
        var year: intmax_t = @as(intmax_t, @bitCast(@as(c_long, tm_1.*.tm_year)));
        _ = sprintf(p, "%04ld-%02d-%02d %02d:%02d:%02d", year + @as(intmax_t, 1900), tm_1.*.tm_mon + 1, tm_1.*.tm_mday, tm_1.*.tm_hour, tm_1.*.tm_min, tm_1.*.tm_sec);
    ...
    _ = sprintf(p, ".%09d", @as(c_int, @bitCast(@as(c_uint, @truncate((ts -% @as(uintmax_t, @bitCast(@as(c_long, 2 + 1)))) & @as(uintmax_t, @bitCast(@as(c_long, (1 << @intCast(if (true) 30 else 0)) - 1))))))));
    p += @as([*c]u8, @ptrFromInt(strlen(p) -% ulong));
...
pub fn file_hash_cmp(arg_x: ?*const anyopaque, arg_y: ?*const anyopaque) callconv(.C) c_int {
    ...
    var y = arg_y;
    while (true) {
        return if (@as(?*const struct_file, @ptrCast(x)).*.hname == @as(?*const struct_file, @ptrCast(y)).*.hname) 0 else strcmp(@as(?*const struct_file, @ptrCast(x)).*.hname, @as(?*const struct_file, @ptrCast(y)).*.hname);
    }
    return 0;
}
...
pub fn snap_file(arg_item: ?*const anyopaque, arg_arg: ?*anyopaque) callconv(.C) void {
    ...
    if (f.*.variables != null) {
        prereqs = expand_extra_prereqs(lookup_variable_in_set(".EXTRA_PREREQS", @sizeOf([15]u8) -% ulong, f.*.variables.*.set));
...
pub fn verify_file(arg_item: ?*const anyopaque) callconv(.C) void {
    ...
    while (true) {
        if (((f.*.name != null) and (@as(c_int, @bitCast(@as(c_uint, f.*.name[0]))) != 0)) and !(strcache_iscached(f.*.name) != 0)) {
            c_error_fn_replaced_for_reserved_keyword(null, (strlen(f.*.name) +% (@sizeOf([5]u8) -% ulong)) +% strlen(f.*.name), gettext("%s: Field '%s' not cached: %s"), f.*.name, "name", f.*.name);
        }
        if (!false) break;
    }
    while (true) {
        if (((f.*.hname != null) and (@as(c_int, @bitCast(@as(c_uint, f.*.hname[0]))) != 0)) and !(strcache_iscached(f.*.hname) != 0)) {
            c_error_fn_replaced_for_reserved_keyword(null, (strlen(f.*.name) +% (@sizeOf([6]u8) -% ulong)) +% strlen(f.*.hname), gettext("%s: Field '%s' not cached: %s"), f.*.name, "hname", f.*.hname);
        }
        if (!false) break;
    }
    while (true) {
        if (((f.*.vpath != null) and (@as(c_int, @bitCast(@as(c_uint, f.*.vpath[0]))) != 0)) and !(strcache_iscached(f.*.vpath) != 0)) {
            c_error_fn_replaced_for_reserved_keyword(null, (strlen(f.*.name) +% (@sizeOf([6]u8) -% ulong)) +% strlen(f.*.vpath), gettext("%s: Field '%s' not cached: %s"), f.*.name, "vpath", f.*.vpath);
        }
        if (!false) break;
    }
    while (true) {
        if (((f.*.stem != null) and (@as(c_int, @bitCast(@as(c_uint, f.*.stem[0]))) != 0)) and !(strcache_iscached(f.*.stem) != 0)) {
            c_error_fn_replaced_for_reserved_keyword(null, (strlen(f.*.name) +% (@sizeOf([5]u8) -% ulong)) +% strlen(f.*.stem), gettext("%s: Field '%s' not cached: %s"), f.*.name, "stem", f.*.stem);
        }
        if (!false) break;
    }
    {
        ...
        while (d != null) : (d = d.*.next) {
            if (!(d.*.need_2nd_expansion != 0)) while (true) {
                if (((d.*.name != null) and (@as(c_int, @bitCast(@as(c_uint, d.*.name[0]))) != 0)) and !(strcache_iscached(d.*.name) != 0)) {
                    c_error_fn_replaced_for_reserved_keyword(null, (strlen(d.*.name) +% (@sizeOf([5]u8) -% ulong)) +% strlen(d.*.name), gettext("%s: Field '%s' not cached: %s"), d.*.name, "name", d.*.name);
                }
                if (!false) break;
            };
            while (true) {
                if (((d.*.stem != null) and (@as(c_int, @bitCast(@as(c_uint, d.*.stem[0]))) != 0)) and !(strcache_iscached(d.*.stem) != 0)) {
                    c_error_fn_replaced_for_reserved_keyword(null, (strlen(d.*.name) +% (@sizeOf([5]u8) -% ulong)) +% strlen(d.*.stem), gettext("%s: Field '%s' not cached: %s"), d.*.name, "stem", d.*.stem);
                }
                if (!false) break;
            }
        }
...
// src/shuffle.h:24:9
pub const MERGE = @compileError("unable to translate macro: undefined identifier `to_file`");
// src/file.c:319:9
pub const VERIFY_CACHED = @compileError("unable to translate C expr: unexpected token 'do'");
// src/file.c:1189:9
pub inline fn EXPANSION_INCREMENT(_l: anytype) @TypeOf((@import("std").zig.c_translation.MacroArithmetic.div(_l, 500) + 1) * 500) {
    return (@import("std").zig.c_translation.MacroArithmetic.div(_l, 500) + 1) * 500;
}
