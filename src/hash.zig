...
pub export fn hash_init(arg_ht: [*c]struct_hash_table, arg_size: c_ulong, arg_hash_1: hash_func_t, arg_hash_2: hash_func_t, arg_hash_cmp: hash_cmp_func_t) void {
    ...
    ht.*.ht_capacity = ht.*.ht_size -% (ht.*.ht_size / ulong);
    ht.*.ht_fill = 0;
    ht.*.ht_collisions = 0;
    ht.*.ht_lookups = 0;
    ht.*.ht_rehashes = 0;
...
pub export fn hash_find_slot(arg_ht: [*c]struct_hash_table, arg_key: ?*const anyopaque) [*c]?*anyopaque {
    ...
    ht.*.ht_lookups +%= 1;
    while (true) {
        hash_1 &= @as(c_uint, @bitCast(@as(c_uint, @truncate(ht.*.ht_size -% ulong))));
        ...
        if (!(hash_2 != 0)) {
            hash_2 = @as(c_uint, @bitCast(@as(c_uint, @truncate(ht.*.ht_hash_2.?(key) | ulong))));
        }
        hash_1 +%= hash_2;
    }
    return null;
...
pub export fn hash_dump(arg_ht: [*c]struct_hash_table, arg_vector_0: [*c]?*anyopaque, arg_compare: qsort_cmp_t) [*c]?*anyopaque {
    ...
    var end: [*c]?*anyopaque = &ht.*.ht_vec[ht.*.ht_size];
    if (vector_0 == null) {
        vector_0 = @as([*c]?*anyopaque, @ptrCast(@alignCast(xmalloc(@sizeOf(?*anyopaque) *% (ht.*.ht_fill +% ulong)))));
    }
    vector = vector_0;
    {
        slot = ht.*.ht_vec;
...
pub export fn jhash(arg_k: [*c]const u8, arg_length: c_int) c_uint {
    ...
    a = blk: {
        const tmp = blk_1: {
            const tmp_2 = 3735928559 +% @as(c_uint, @bitCast(length));
            c = tmp_2;
            break :blk_1 tmp_2;
        };
        b = tmp;
    ...
    while (length > 12) {
        while (true) {
            ...
            _ = memcpy(@as(?*anyopaque, @ptrCast(&val)), @as(?*const anyopaque, @ptrCast(k)), ulong);
            a +%= val;
            if (!false) break;
        }
        while (true) {
            var val: c_uint = undefined;
            _ = memcpy(@as(?*anyopaque, @ptrCast(&val)), @as(?*const anyopaque, @ptrCast(k + @as(usize, @bitCast(@as(isize, 4))))), ulong);
            b +%= val;
            if (!false) break;
        }
        while (true) {
            var val: c_uint = undefined;
            _ = memcpy(@as(?*anyopaque, @ptrCast(&val)), @as(?*const anyopaque, @ptrCast(k + @as(usize, @bitCast(@as(isize, 8))))), ulong);
            c +%= val;
            if (!false) break;
        }
        {
            a -%= c;
            a ^= (c << 4) | (c >> @intCast(32 - 4));
            c +%= b;
            b -%= a;
            b ^= (a << 6) | (a >> @intCast(32 - 6));
            a +%= c;
            c -%= b;
            c ^= (b << 8) | (b >> @intCast(32 - 8));
            b +%= a;
            a -%= c;
            a ^= (c << 16) | (c >> @intCast(32 - 16));
            c +%= b;
            b -%= a;
            b ^= (a << 19) | (a >> @intCast(32 - 19));
            a +%= c;
            c -%= b;
            c ^= (b << 4) | (b >> @intCast(32 - 4));
            b +%= a;
        }
        length -= 12;
        k += @as(usize, @bitCast(@as(isize, 12)));
    }
    ...
    if (length == 4) {
        c +%= @as(c_uint, @bitCast(@as(c_uint, k[3]))) << 24;
    }
    if (length >= 3) {
        c +%= @as(c_uint, @bitCast(@as(c_uint, k[2]))) << 16;
    }
    if (length >= 2) {
        c +%= @as(c_uint, @bitCast(@as(c_uint, k[1]))) << 8;
    }
    c +%= @as(c_uint, @bitCast(@as(c_uint, k[0])));
    {
        c ^= b;
        c -%= (b << 14) | (b >> @intCast(32 - 14));
        a ^= c;
        a -%= (c << 11) | (c >> @intCast(32 - 11));
        b ^= a;
        b -%= (a << 25) | (a >> @intCast(32 - 25));
        c ^= b;
        c -%= (b << 16) | (b >> @intCast(32 - 16));
        a ^= c;
        a -%= (c << 4) | (c >> @intCast(32 - 4));
        b ^= a;
        b -%= (a << 14) | (a >> @intCast(32 - 14));
        c ^= b;
        c -%= (b << 24) | (b >> @intCast(32 - 24));
    }
    return c;
}
pub export fn jhash_string(arg_k: [*c]const u8) c_uint {
    ...
    while (true) {
        while (true) {
            ...
            have_nul = ((val -% uint) & ~val) & 2155905152;
            if (!(have_nul != 0)) {
                a +%= val;
            } else if ((val & uint) != 0) {
                if ((val & uint) == uint) {
                    a +%= val & uint;
                } else if ((val & uint) == uint) {
                    a +%= val & uint;
                } else {
                    a +%= val;
                }
...
// src/hash.c:24:9
pub inline fn rol32(v: anytype, n: anytype) @TypeOf((v << n) | (v >> (32 - n))) {
    return (v << n) | (v >> (32 - n));
}
