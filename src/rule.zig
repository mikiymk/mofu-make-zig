...
pub export fn install_pattern_rule(arg_p: [*c]struct_pspec, arg_terminal: c_int) void {
    ...
    r.*.suffixes[0] += 1;
    ptr = p.*.dep;
    r.*.deps = @as(?*struct_dep, @ptrCast(parse_file_seq(@as([*c][*c]u8, @ptrCast(@alignCast(&ptr))), @sizeOf(struct_dep), 1, null, 0)));
    ...
    if (r.*._defn == @as([*c]u8, @ptrCast(@alignCast(@as(?*anyopaque, @ptrFromInt(0)))))) {
        ...
        var ood: ?*const struct_dep = null;
        {
            ...
            while (k < @as(c_uint, @bitCast(@as(c_uint, r.*.num)))) : (k +%= 1) {
                len +%= @as(usize, @bitCast(@as(c_ulong, r.*.lens[k] +% uint)));
            }
        }
        {
            dep_1 = r.*.deps;
            while (dep_1 != null) : (dep_1 = dep_1.*.next) {
                len +%= @as(usize, @bitCast((strlen(if (dep_1.*.name != null) dep_1.*.name else dep_1.*.file.*.name) +% (if (@as(c_int, @bitCast(dep_1.*.wait_here)) != 0) @sizeOf([7]u8) -% ulong else ulong)) +% ulong));
            }
        }
        p = blk: {
        ...
        {
            dep_1 = r.*.deps;
            while (dep_1 != null) : (dep_1 = dep_1.*.next) if (@as(c_int, @bitCast(dep_1.*.ignore_mtime)) == 0) {
                if (dep_1.*.wait_here != 0) {
                    p = @as([*c]u8, @ptrCast(@alignCast(mempcpy(@as(?*anyopaque, @ptrCast(p)), @as(?*const anyopaque, @ptrCast(" .WAIT")), @sizeOf([7]u8) -% ulong))));
                }
                p = @as([*c]u8, @ptrCast(@alignCast(mempcpy(mempcpy(@as(?*anyopaque, @ptrCast(p)), @as(?*const anyopaque, @ptrCast(" ")), ulong), @as(?*const anyopaque, @ptrCast(if (dep_1.*.name != null) dep_1.*.name else dep_1.*.file.*.name)), strlen(if (dep_1.*.name != null) dep_1.*.name else dep_1.*.file.*.name)))));
            } else if (ood == null) {
                ood = dep_1;
            };
        }
...
pub export fn print_rule_data_base() void {
    ...
    if (num_pattern_rules != rules) {
        if (num_pattern_rules != uint) {
            fatal(@as([*c]floc, @ptrFromInt(0)), (((ulong *% @sizeOf(uintmax_t)) / ulong) +% ulong) *% ulong, gettext("BUG: num_pattern_rules is wrong!  %u != %u"), num_pattern_rules, rules);
        }
    }
}
