...
pub extern fn c_error_fn_replaced_for_reserved_keyword(flocp: [*c]const floc, len: usize, fmt: [*c]const u8, ...) void;
...
pub export fn perror_with_name(arg_str: [*c]const u8, arg_name: [*c]const u8) void {
    ...
    var name = arg_name;
    var err: [*c]const u8 = strerror(__errno_location().*);
    c_error_fn_replaced_for_reserved_keyword(@as([*c]floc, @ptrFromInt(0)), (strlen(str) +% strlen(name)) +% strlen(err), gettext("%s%s: %s"), str, name, err);
}
...
pub export fn output_start() void {
    if ((output_context != null) and (@as(c_int, @bitCast(output_context.*.syncout)) != 0)) if (!((output_context.*.out >= 0) or (output_context.*.err >= 0))) {
...
pub export fn output_dump(arg_out: ?*struct_output) void {
    var out = arg_out;
    var outfd_not_empty: c_int = @intFromBool((out.*.out != -1) and (lseek(out.*.out, @as(__off_t, 0), 2) > @as(__off_t, 0)));
    var errfd_not_empty: c_int = @intFromBool((out.*.err != -1) and (lseek(out.*.err, @as(__off_t, 0), 2) > @as(__off_t, 0)));
    if ((outfd_not_empty != 0) or (errfd_not_empty != 0)) {
        var traced: c_int = 0;
        if (!(osync_acquire() != 0)) {
            c_error_fn_replaced_for_reserved_keyword(@as([*c]floc, @ptrFromInt(0)), @as(usize, 0), gettext("warning: Cannot acquire output lock, disabling output sync."));
            osync_clear();
        }
...
pub fn _outputs(out: ?*struct_output, is_err: c_int, msg: [*c]const u8) callconv(.C) void {
    var f: [*c]FILE = undefined;
    if ((out != null) and (@as(c_int, @bitCast(out.*.syncout)) != 0)) {
        const fd: c_int = if (is_err != 0) out.*.err else out.*.out;
        if (fd != -1) {
...
pub fn log_working_directory(arg_entering: c_int) callconv(.C) c_int {
    ...
    need = ((strlen(program) +% (((ulong *% @sizeOf(uintmax_t)) / ulong) +% ulong)) +% ulong) +% ulong;
    if (starting_directory != null) {
        need +%= @as(usize, @bitCast(strlen(starting_directory)));
    }
    if (makelevel == uint) if (starting_directory == null) if (entering != 0) {
    ...
    if (makelevel == uint) if (starting_directory == null) {
...
pub fn pump_from_tmp(arg_from: c_int, arg_to: [*c]FILE) callconv(.C) void {
    ...
    while (true) {
        ...
        if (len <= 0) break;
        if (fwrite(@as(?*const anyopaque, @ptrCast(@as([*c]u8, @ptrCast(@alignCast(&buffer.static))))), @as(c_ulong, @bitCast(@as(c_long, len))), ulong, to) < ulong) {
            perror("fwrite()");
            break;
        }
        _ = fflush(to);
...
pub inline fn OUTPUT_ISSET(_out: anytype) @TypeOf((_out.*.out >= 0) or (_out.*.err >= 0)) {
    return (_out.*.out >= 0) or (_out.*.err >= 0);
}
pub inline fn FD_NOT_EMPTY(_f: anytype) @TypeOf((_f != OUTPUT_NONE) and (lseek(_f, 0, SEEK_END) > 0)) {
    return (_f != OUTPUT_NONE) and (lseek(_f, 0, SEEK_END) > 0);
}
...
pub const fmtstring = struct_fmtstring;
