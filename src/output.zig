...
pub fn FD_STDOUT() c_int {
    return c_stdio.fileno(c_stdio.stdout);
}
...
/// Our goal here is to be sure we don't try to allocate more memory,
/// which means we don't want to use string translations or normal cleanup.
pub fn out_of_memory() noreturn {
    const stdout = std.io.getStdOut();

    _ = stdout.write(main.program) catch {};
    _ = stdout.write(": *** virtual memory exhausted\n") catch {};
    std.posix.exit(makeint.MAKE_FAILURE);
}
