...
pub export fn new_job(arg_file_1: ?*struct_file) void {
    ...
    while (true) {
        output_context = if (@as(c_int, @bitCast((&c.*.output).*.syncout)) != 0) &c.*.output else null;
        if (!false) break;
    }
    ...
    if ((job_slots == @as(c_uint, @bitCast(@as(c_int, 1)))) or (not_parallel != 0)) while (@as(c_int, @bitCast(file_1.*.command_state)) == cs_running) {
        reap_children(@as(c_int, 1), @as(c_int, 0));
    };
    while (true) {
        output_context = null;
...
pub fn child_error(arg_child_1: ?*struct_child, arg_exit_code: c_int, arg_exit_sig: c_int, arg_coredump: c_int, arg_ignored: c_int) callconv(.C) void {
    ...
    var dump: [*c]const u8 = "";
    _ = &dump;
    var f: ?*const struct_file = child_1.*.file;
    _ = &f;
    ...
    while (true) {
        output_context = if (@as(c_int, @bitCast((&child_1.*.output).*.syncout)) != 0) &child_1.*.output else null;
        if (!false) break;
    }
    show_goal_error();
...
pub inline fn WAIT_NOHANG(status: anytype) @TypeOf(waitpid(-@as(c_int, 1), status, WNOHANG)) {
    _ = &status;
    return waitpid(-@as(c_int, 1), status, WNOHANG);
}
pub const _SPAWN_H = @as(c_int, 1);
...
pub const PROC_LOADAVG_SIZE = @as(c_int, 64);
pub const PRESERVE_BSNL = @as(c_int, 1);
