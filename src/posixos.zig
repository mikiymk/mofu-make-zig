...
pub fn STREAM_OK(_s: std.posix.fd_t) bool {
    _ = std.posix.fcntl(_s, std.posix.F.GETFD, 0) catch |e| switch (e) {
        .EBADF => return false,
        else => return true,
    };
    return true;
...
pub fn check_io_state() c_uint {
    const state = struct {
        pub var state: c_uint = os.IO_UNKNOWN;
    };

    // We only need to compute this once per process.
    if (state.state != os.IO_UNKNOWN) {
        return state.state;
    }

    if (STREAM_OK(std.posix.STDIN_FILENO)) {
        state.state |= os.IO_STDIN_OK;
    }
    if (STREAM_OK(std.posix.STDOUT_FILENO)) {
        state.state |= os.IO_STDOUT_OK;
    }
    if (STREAM_OK(std.posix.STDERR_FILENO)) {
        state.state |= os.IO_STDERR_OK;
    }

    if (makeint.ALL_SET(state.state, os.IO_STDOUT_OK | os.IO_STDERR_OK)) {
        if (std.posix.fstat(std.posix.STDOUT_FILENO)) |stbuf_o| {
            if (std.posix.fstat(std.posix.STDERR_FILENO)) |stbuf_e| {
                if (stbuf_o.st_dev == stbuf_e.st_dev and
                    stbuf_o.st_ino == stbuf_e.st_ino)
                {
                    state.state |= os.IO_COMBINED_OUTERR;
                }
            } else |_| {}
        } else |_| {}
    }
...
pub fn fd_set_append(fd: std.fs.File) void {
    _ = fd;
