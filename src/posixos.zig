...
pub export fn check_io_state() c_uint {
    ...
    if (state.static != uint) return state.static;
    ...
    if ((state.static & @as(c_uint, @bitCast(8 | 16))) == @as(c_uint, @bitCast(8 | 16))) {
...
pub export fn fd_set_append(arg_fd: c_int) void {
    ...
    var flags: c_int = undefined;
    if ((fstat(fd, &stbuf) == 0) and ((stbuf.st_mode & @as(__mode_t, 61440)) == @as(__mode_t, 32768))) {
        flags = fcntl(fd, 3, 0);
        if (flags >= 0) {
            var r: c_int = undefined;
            while (((blk: {
                const tmp = fcntl(fd, 4, flags | 1024);
                r = tmp;
                break :blk tmp;
            }) == -1) and (__errno_location().* == 4)) {}
        }
...
pub export fn os_anontmp() c_int {
    ...
    if (tmpfile_works.static != 0) {
        while (((blk: {
            const tmp = open(tdir, (2 | (4194304 | 65536)) | 128, 384);
            fd = tmp;
            break :blk tmp;
        }) == -1) and (__errno_location().* == 4)) {}
        if (fd >= 0) return fd;
    ...
    if ((tdir == @as([*c]const u8, @ptrCast(@alignCast("/tmp")))) or ((@as(c_int, @bitCast(@as(c_uint, tdir.*))) == @as(c_int, @bitCast(@as(c_uint, "/tmp".*)))) and ((@as(c_int, @bitCast(@as(c_uint, tdir.*))) == @as(c_int, '\x00')) or !(strcmp(tdir + @as(usize, @bitCast(@as(isize, 1))), "/tmp" + @as(usize, @bitCast(@as(isize, 1)))) != 0)))) {
        var mask: mode_t = umask(@as(__mode_t, 63));
        ...
        if (!(tfile != null)) {
            c_error_fn_replaced_for_reserved_keyword(@as([*c]floc, @ptrFromInt(0)), strlen(strerror(__errno_location().*)), "tmpfile: %s", strerror(__errno_location().*));
            return -1;
        }
        _ = umask(mask);
        while (((blk: {
        ...
        if (fd < 0) {
            c_error_fn_replaced_for_reserved_keyword(@as([*c]floc, @ptrFromInt(0)), strlen(strerror(__errno_location().*)), "dup: %s", strerror(__errno_location().*));
        }
        _ = fclose(tfile);
    }
    return fd;
...
pub export fn jobserver_setup(arg_slots: c_int, arg_style: [*c]const u8) c_uint {
    ...
    if (!(style != null) or (strcmp(style, "fifo") == 0)) {
        ...
        fifo_name = @as([*c]u8, @ptrCast(@alignCast(xmalloc(((strlen(tmpdir) +% (@sizeOf([7]u8) -% ulong)) +% (((ulong *% @sizeOf(uintmax_t)) / ulong) +% ulong)) +% ulong))));
        _ = sprintf(fifo_name, "%s/GMfifo%lld", tmpdir, @as(c_longlong, @bitCast(@as(c_longlong, make_pid()))));
        while (((blk: {
            const tmp = mkfifo(fifo_name, @as(__mode_t, 384));
            r = tmp;
            break :blk tmp;
        }) == -1) and (__errno_location().* == 4)) {}
        if (r < 0) {
            ...
            while (((blk: {
                const tmp = open(fifo_name, 0 | 2048);
                job_fds[0] = tmp;
                break :blk tmp;
            }) == -1) and (__errno_location().* == 4)) {}
            if (job_fds[0] < 0) {
...
pub export fn jobserver_parse_auth(arg_auth: [*c]const u8) c_uint {
    ...
    if (strncmp(auth, "fifo:", @sizeOf([6]u8) -% ulong) == 0) {
        fifo_name = xstrdup(auth + (@sizeOf([6]u8) -% ulong));
        while (((blk: {
        ...
        if (job_fds[0] < 0) {
            c_error_fn_replaced_for_reserved_keyword(@as([*c]floc, @ptrFromInt(0)), strlen(fifo_name) +% strlen(strerror(__errno_location().*)), gettext("cannot open jobserver %s: %s"), fifo_name, strerror(__errno_location().*));
            return 0;
        }
        while (((blk: {
            const tmp = open(fifo_name, 1);
        ...
        c_error_fn_replaced_for_reserved_keyword(@as([*c]floc, @ptrFromInt(0)), strlen(auth), gettext("invalid --jobserver-auth string '%s'"), auth);
        return 0;
    }
    if (make_job_rfd() < 0) {
...
pub export fn jobserver_get_auth() [*c]u8 {
    ...
    if (js_type == @as(c_uint, @bitCast(js_fifo))) {
        auth = @as([*c]u8, @ptrCast(@alignCast(xmalloc((strlen(fifo_name) +% (@sizeOf([6]u8) -% ulong)) +% ulong))));
        _ = sprintf(auth, "fifo:%s", fifo_name);
    } else {
        auth = @as([*c]u8, @ptrCast(@alignCast(xmalloc(((((ulong *% @sizeOf(uintmax_t)) / ulong) +% ulong) *% ulong) +% ulong))));
...
pub export fn jobserver_acquire(arg_timeout: c_int) c_uint {
    ...
    while (true) {
        ...
        _ = blk: {
            ...
            ref.* |= @as(__fd_mask, @bitCast(1 << @intCast(@import("std").zig.c_translation.signedRemainder(job_fds[0], 8 * @as(c_int, @bitCast(@as(c_uint, @truncate(@sizeOf(__fd_mask)))))))));
            break :blk ref.*;
        };
        ...
        if (r < 0) {
            if (__errno_location().* == 11) continue;
...
pub export fn osync_get_mutex() [*c]u8 {
    var mutex: [*c]u8 = null;
    if (osync_enabled() != 0) {
        mutex = @as([*c]u8, @ptrCast(@alignCast(xmalloc((strlen(osync_tmpfile) +% (@sizeOf([5]u8) -% ulong)) +% ulong))));
...
pub export fn osync_parse_mutex(arg_mutex: [*c]const u8) c_uint {
    var mutex = arg_mutex;
    if (strncmp(mutex, "fnm:", @sizeOf([5]u8) -% ulong) != 0) {
        c_error_fn_replaced_for_reserved_keyword(@as([*c]floc, @ptrFromInt(0)), strlen(mutex), gettext("invalid --sync-mutex string '%s'"), mutex);
        return 0;
    }
    free(@as(?*anyopaque, @ptrCast(osync_tmpfile)));
    osync_tmpfile = xstrdup(mutex + (@sizeOf([5]u8) -% ulong));
    while (((blk: {
...
pub var job_rfd: c_int = -1;
pub var token: u8 = '+';
pub const js_none: c_int = 0;
pub const js_pipe: c_int = 1;
pub const js_fifo: c_int = 2;
...
pub fn set_blocking(arg_fd: c_int, arg_blocking: c_int) callconv(.C) void {
    ...
    if (flags >= 0) {
        var r: c_int = undefined;
        flags = if (blocking != 0) flags & ~2048 else flags | 2048;
        while (((blk: {
...
pub inline fn FD_OK(_f: anytype) @TypeOf(fcntl(_f, F_GETFD) != -1) {
    return fcntl(_f, F_GETFD) != -1;
}
...
pub inline fn STREAM_OK(_s: anytype) @TypeOf((fcntl(fileno(_s), F_GETFD) != -1) or (errno != EBADF)) {
    return (fcntl(fileno(_s), F_GETFD) != -1) or (errno != EBADF);
}
pub const FIFO_PREFIX = "fifo:";
pub const FNAME_PREFIX = "GMfifo";
