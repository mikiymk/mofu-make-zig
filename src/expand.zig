...
pub export fn variable_buffer_output(arg_ptr: [*c]u8, arg_string: [*c]const u8, arg_length: usize) [*c]u8 {
    ...
    if ((newlen +% @as(usize, 5)) > variable_buffer_length) {
        ...
        variable_buffer_length = if ((newlen +% @as(usize, 100)) > (@as(usize, 2) *% variable_buffer_length)) newlen +% @as(usize, 100) else @as(usize, 2) *% variable_buffer_length;
...
pub export fn variable_expand(arg_line: [*c]const u8) [*c]u8 {
    var line = arg_line;
    return variable_expand_string(null, line, 18446744073709551615);
}
...
pub export fn recursively_expand_for_file(arg_v: ?*struct_variable, arg_file_1: ?*struct_file) [*c]u8 {
    ...
    if ((@as(c_int, @bitCast(v.*.expanding)) != 0) and (env_recursion != 0)) {
        ...
        {
            ep = environ;
            while (ep.* != null) : (ep += 1) if ((@as(c_int, @bitCast(@as(c_uint, ep.*[nl]))) == @as(c_int, '=')) and (strncmp(ep.*, v.*.name, nl) == 0)) return xstrdup((ep.* + nl) + @as(usize, @bitCast(@as(isize, 1))));
        }
        return xstrdup("");
    }
    ...
    if (v.*.append != 0) {
        ...
        value = allocated_variable_expand_for_file(v.*.value, @as(?*struct_file, @ptrFromInt(0)));
    }
    v.*.expanding = 0;
    if (set_reading != 0) {
        reading_file = null;
...
pub const VARIABLE_BUFFER_ZONE = 5;
