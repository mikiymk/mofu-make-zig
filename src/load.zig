...
pub const struct_link_map_39 = opaque {};
pub const struct_dl_find_object = extern struct {
    ...
    dlfo_link_map: ?*struct_link_map_39 = @import("std").mem.zeroes(?*struct_link_map_39),
...
pub const enum_cmd_state_40 = c_uint;
...
pub fn load_object(arg_flocp: [*c]const floc, arg_noerror: c_int, arg_ldname: [*c]const u8, arg_symname: [*c]const u8) callconv(.C) load_func_t {
    ...
    if (!(global_dl.static != null)) {
        global_dl.static = dlopen(null, 2 | 256);
        if (!(global_dl.static != null)) {
    ...
    if (!(symp != null)) {
        ...
        if (!(strchr(ldname, @as(c_int, '/')) != null)) {
            dlp = dlopen(concat(uint, "./", ldname), 1 | 256);
        }
        if (!(dlp != null)) {
            dlp = dlopen(ldname, 1 | 256);
        }
        if (!(dlp != null)) {
            var err: [*c]const u8 = dlerror();
            if (noerror != 0) {
                ...
                c_error_fn_replaced_for_reserved_keyword(flocp, strlen(err), "%s", err);
            }
            return null;
        }
        while (true) {
...
pub const DLFO_STRUCT_HAS_EH_DBASE = 0;
pub const DLFO_STRUCT_HAS_EH_COUNT = 0;
pub const DLFO_EH_SEGMENT_TYPE = @compileError("unable to translate macro: undefined identifier `PT_GNU_EH_FRAME`");
// /nix/store/1l5gb4773rbqjzv49wb4h9xlmaz2zs5h-zig-0.13.0/lib/zig/libc/include/x86_64-linux-gnu/bits/dl_find_object.h:29:9
pub const LM_ID_BASE = 0;
pub const LM_ID_NEWLM = -1;
pub const RTLD_NEXT = @import("std").zig.c_translation.cast(?*anyopaque, -1);
pub const RTLD_DEFAULT = @import("std").zig.c_translation.cast(?*anyopaque, 0);
...
pub const load_list = struct_load_list;
