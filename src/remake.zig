...
pub export fn show_goal_error() void {
    ...
    if ((@as(c_int, @bitCast(goal_dep.*.flags)) & ((1 << 1) | (1 << 2))) != (1 << 1)) return;
    {
        goal = goal_list;
        while (goal != null) : (goal = goal.*.next) if (goal_dep.*.file == goal.*.file) {
            if (goal.*.c_error_fn_replaced_for_reserved_keyword != 0) {
                c_error_fn_replaced_for_reserved_keyword(&goal.*.floc, strlen(goal.*.file.*.name) +% strlen(strerror(goal.*.c_error_fn_replaced_for_reserved_keyword)), "%s: %s", goal.*.file.*.name, strerror(goal.*.c_error_fn_replaced_for_reserved_keyword));
                goal.*.c_error_fn_replaced_for_reserved_keyword = 0;
            }
            return;
        };
    }
...
pub export fn update_goal_chain(arg_goaldeps: ?*struct_goaldep) enum_update_status_37 {
    ...
    var status: enum_update_status_37 = @as(c_uint, @bitCast(us_none));
    ...
    while (goals != null) {
        ...
        while (gu != null) {
            ...
            goal_dep = g;
            {
                ...
                while (file_1 != @as(?*struct_file, @ptrCast(@as(?*anyopaque, @ptrFromInt(0))))) : (file_1 = file_1.*.prev) {
                    var ocommands_started: c_uint = undefined;
                    var fail: enum_update_status_37 = undefined;
                    file_1.*.dontcare = @as(c_uint, @intFromBool((@as(c_int, @bitCast(g.*.flags)) & (1 << 2)) != 0));
            ...
            if ((stop != 0) or !(any_not_updated != 0)) {
                if ((((!(rebuilding_makefiles != 0) and (@as(c_int, @bitCast(file_1.*.update_status)) == us_success)) and !(g.*.changed != 0)) and !(run_silent != 0)) and !(question_flag != 0)) {
                    message(1, strlen(file_1.*.name), if ((@as(c_int, @bitCast(file_1.*.phony)) != 0) or (file_1.*.cmds == null)) gettext("Nothing to be done for '%s'.") else gettext("'%s' is up to date."), file_1.*.name);
                }
                if (lastgoal == null) {
                    goals = gu.*.next;
                } else {
...
pub fn update_file(arg_file_1: ?*struct_file, arg_depth: c_uint) callconv(.C) enum_update_status_37 {
    var file_1 = arg_file_1;
    var depth = arg_depth;
    var status: enum_update_status_37 = @as(c_uint, @bitCast(us_success));
    ...
    while (f != null) : (f = f.*.prev) {
        var new: enum_update_status_37 = undefined;
...
pub extern fn update_file_1(arg_file_1: ?*struct_file, arg_depth: c_uint) callconv(.C) enum_update_status_37;
pub fn check_dep(arg_file_1: ?*struct_file, arg_depth: c_uint, arg_this_mtime: uintmax_t, arg_must_make_ptr: [*c]c_int) callconv(.C) enum_update_status_37 {
    var file_1 = arg_file_1;
    var depth = arg_depth;
    ...
    var dep_status: enum_update_status_37 = @as(c_uint, @bitCast(us_success));
    _ = blk: {
        const tmp = uint;
        ...
        if ((mtime != @as(uintmax_t, 1)) and (mtime > this_mtime)) {
            ...
            while (d != null) {
                ...
                if ((if (d.*.file.*.double_colon != null) d.*.file.*.double_colon else d.*.file).*.updating != 0) {
                    c_error_fn_replaced_for_reserved_keyword(@as([*c]floc, @ptrFromInt(0)), strlen(file_1.*.name) +% strlen(d.*.file.*.name), gettext("Circular %s <- %s dependency dropped."), file_1.*.name, d.*.file.*.name);
                    if (ld == null) {
                        file_1.*.deps = d.*.next;
                ...
                new = check_dep(d.*.file, depth +% uint, this_mtime, &maybe_make);
                if (new > dep_status) {
                    dep_status = new;
                }
                if (!(d.*.ignore_mtime != 0)) {
...
pub fn touch_file(arg_file_1: ?*struct_file) callconv(.C) enum_update_status_37 {
    var file_1 = arg_file_1;
    if (!(run_silent != 0)) {
    ...
    if (ar_name(file_1.*.name) != 0) return @as(c_uint, @bitCast(if (ar_touch(file_1.*.name) != 0) us_failed else us_success)) else {
        ...
        while (((blk: {
            const tmp = open(file_1.*.name, 2 | 64, 438);
            fd = tmp;
            break :blk tmp;
        ...
        if (fd < 0) {
            ...
            if (statbuf.st_size == @as(__off_t, 0)) {
                _ = close(fd);
                while (((blk: {
                    const tmp = open(file_1.*.name, 2 | 512, 438);
                    fd = tmp;
                    break :blk tmp;
                }) == -1) and (__errno_location().* == 4)) {}
                if (fd < 0) while (true) {
...
pub fn name_mtime(arg_name: [*c]const u8) callconv(.C) uintmax_t {
    ...
    } else if ((__errno_location().* == 2) or (__errno_location().* == 20)) {
        mtime = 1;
    } else {
        perror_with_name("stat: ", name);
        return 1;
    }
    if ((check_symlink_flag != 0) and (strlen(name) <= ulong)) {
        ...
        while (true) {
            ...
            if (e != 0) {
                if ((__errno_location().* != 2) and (__errno_location().* != 20)) {
                    perror_with_name("lstat: ", @as([*c]u8, @ptrCast(@alignCast(&lpath))));
                }
                break;
            }
            if (!((st.st_mode & @as(__mode_t, 61440)) == @as(__mode_t, 40960))) break;
            ltime = file_timestamp_cons(@as([*c]u8, @ptrCast(@alignCast(&lpath))), st.st_mtim.tv_sec, st.st_mtim.tv_nsec);
            if (ltime > mtime) {
                mtime = ltime;
            }
            while (((blk: {
                const tmp = readlink(@as([*c]u8, @ptrCast(@alignCast(&lpath))), @as([*c]u8, @ptrCast(@alignCast(&lbuf))), @as(usize, @bitCast(@as(c_long, 4096 - 1))));
                llen = tmp;
                break :blk tmp;
            ...
            } else if (((@divExact(@as(c_long, @bitCast(@intFromPtr(p) -% @intFromPtr(@as([*c]u8, @ptrCast(@alignCast(&lpath)))))), @sizeOf(u8)) + llen) + long) > long) break else {
                _ = strcpy(p + @as(usize, @bitCast(@as(isize, 1))), @as([*c]u8, @ptrCast(@alignCast(&lbuf))));
            }
        }
    }
    return mtime;
...
pub fn library_search(arg_lib: [*c]const u8, arg_mtime_ptr: [*c]uintmax_t) callconv(.C) [*c]const u8 {
    ...
    lib += @as(usize, @bitCast(@as(isize, 2)));
    liblen = strlen(lib);
    p2 = libpatterns;
    while ((blk: {
        ...
        var libbuf: [*c]u8 = undefined;
        {
            ...
            p3 = find_percent(p);
            if (!(p3 != null)) {
                c_error_fn_replaced_for_reserved_keyword(@as([*c]floc, @ptrFromInt(0)), strlen(p), gettext(".LIBPATTERNS element '%s' is not a pattern"), p);
                p[len] = c;
                continue;
            }
            ...
            p4 = variable_buffer_output(p4, p3 + @as(usize, @bitCast(@as(isize, 1))), len -% @as(usize, @bitCast(@divExact(@as(c_long, @bitCast(@intFromPtr(p3) -% @intFromPtr(p))), @sizeOf(u8)))));
            p[len] = c;
        ...
        {
            var vpath_index: c_uint = ~uint -% std_dirs.static;
            {
                dp = @as([*c][*c]const u8, @ptrCast(@alignCast(&dirs.static)));
                while (dp.* != null) : (dp += 1) {
...
pub fn check_also_make(arg_file_1: ?*const struct_file) callconv(.C) void {
    ...
    if (((mtime >= @as(uintmax_t, @bitCast(@as(c_long, 2 + 1)))) and (mtime <= (((((((~@as(uintmax_t, 0) -% (if (!(@as(uintmax_t, @bitCast(@as(c_long, -1))) <= @as(uintmax_t, 0))) @as(uintmax_t, 0) else ~@as(uintmax_t, 0) << @intCast((@sizeOf(uintmax_t) *% ulong) -% ulong))) -% @as(uintmax_t, @bitCast(@as(c_long, 2 + 1)))) >> @intCast(if (true) 30 else 0)) << @intCast(if (true) 30 else 0)) +% @as(uintmax_t, @bitCast(@as(c_long, 2 + 1)))) +% @as(uintmax_t, @bitCast(@as(c_long, if (true) 1000000000 else 1)))) -% @as(uintmax_t, 1)))) and (mtime > file_1.*.mtime_before_update)) {
        ad = file_1.*.also_make;
        while (ad != null) : (ad = ad.*.next) if (ad.*.file.*.last_mtime == @as(uintmax_t, 1)) {
            c_error_fn_replaced_for_reserved_keyword(if (file_1.*.cmds != null) &file_1.*.cmds.*.fileinfo else @as([*c]floc, @ptrFromInt(0)), strlen(ad.*.file.*.name), gettext("warning: pattern recipe did not update peer target '%s'."), ad.*.file.*.name);
        };
    }
}
pub fn complain(arg_file_1: ?*struct_file) callconv(.C) void {
    ...
    if (d == null) {
        ...
        if (file_1.*.parent != null) {
            var l: usize = (strlen(file_1.*.name) +% strlen(file_1.*.parent.*.name)) +% ulong;
            ...
            c_error_fn_replaced_for_reserved_keyword(@as([*c]floc, @ptrFromInt(0)), l, m, "*** ", file_1.*.name, file_1.*.parent.*.name, ".");
        } else {
            var l: usize = strlen(file_1.*.name) +% ulong;
            var m: [*c]const u8 = gettext("%sNo rule to make target '%s'%s");
            if (!(keep_going_flag != 0)) {
                fatal(@as([*c]floc, @ptrFromInt(0)), l, m, "", file_1.*.name, "");
            }
            c_error_fn_replaced_for_reserved_keyword(@as([*c]floc, @ptrFromInt(0)), l, m, "*** ", file_1.*.name, ".");
        }
        file_1.*.no_diag = 0;
    }
}
...
pub inline fn MTIME(file_1: anytype) @TypeOf(if (rebuilding_makefiles) file_mtime_no_search(file_1) else file_mtime(file_1)) {
