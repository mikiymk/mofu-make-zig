...
pub export fn define_default_variables() void {
    ...
    if (no_builtin_variables_flag != 0) return;
    {
        ...
        while (s.* != null) : (s += @as(usize, @bitCast(@as(isize, 2)))) {
...
pub export fn set_default_suffixes() void {
    ...
    if (no_builtin_rules_flag != 0) {
        _ = define_variable_in_set("SUFFIXES", @sizeOf([9]u8) -% ulong, "", @as(c_uint, @bitCast(o_default)), 0, current_variable_set_list.*.set, @as([*c]floc, @ptrFromInt(0)));
    } else {
        var d: ?*struct_dep = undefined;
        var p: [*c]const u8 = @as([*c]u8, @ptrCast(@alignCast(&default_suffixes)));
        suffix_file.*.deps = enter_prereqs(@as(?*struct_dep, @ptrCast(parse_file_seq(@as([*c][*c]u8, @ptrCast(@alignCast(&p))), @sizeOf(struct_dep), 1, null, 0))), null);
        {
            d = suffix_file.*.deps;
        ...
        _ = define_variable_in_set("SUFFIXES", @sizeOf([9]u8) -% ulong, @as([*c]u8, @ptrCast(@alignCast(&default_suffixes))), @as(c_uint, @bitCast(o_default)), 0, current_variable_set_list.*.set, @as([*c]floc, @ptrFromInt(0)));
    }
}
pub export fn install_default_suffix_rules() void {
    ...
    if (no_builtin_rules_flag != 0) return;
    {
        ...
        while (s.* != null) : (s += @as(usize, @bitCast(@as(isize, 2)))) {
            var f: ?*struct_file = enter_file(strcache_add(s[0]));
            if (!(f.*.cmds != null)) {
