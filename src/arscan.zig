...
pub export fn ar_name_equal(arg_name: [*c]const u8, arg_mem: [*c]const u8, arg_truncated: c_int) c_int {
    ...
    if (p != null) {
        name = p + @as(usize, @bitCast(@as(isize, 1)));
    }
    if (truncated != 0) {
        var hdr: struct_ar_hdr = undefined;
        return @intFromBool(strncmp(name, mem, @sizeOf([16]u8) -% ulong) == 0);
    }
...
pub const LOCK_NB = 4;
pub const LOCK_UN = 8;
pub const LOCK_MAND = 32;
pub const LOCK_READ = 64;
pub const LOCK_WRITE = 128;
pub const LOCK_RW = 192;
...
pub const RWH_WRITE_LIFE_NOT_SET = 0;
...
pub inline fn __OPEN_NEEDS_MODE(oflag: anytype) @TypeOf(((oflag & O_CREAT) != 0) or ((oflag & __O_TMPFILE) == __O_TMPFILE)) {
...
pub const SARMAG = 8;
pub const ARFMAG = "`\n";
pub inline fn TOCHAR(_m: anytype) @TypeOf(_m) {
    return _m;
}
...
pub const ARNAME_MAX = @compileError("unable to translate macro: undefined identifier `member_header`");
// src/arscan.c:565:10
pub const PARSE_INT = @compileError("unable to translate macro: undefined identifier `member_header`");
// src/arscan.c:745:9
