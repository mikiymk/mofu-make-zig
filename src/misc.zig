...
pub export fn make_toui(arg_str: [*c]const u8, arg_error_1: [*c][*c]const u8) c_uint {
    ...
    var val: c_ulong = strtoul(str, &end, 10);
    if (error_1 != null) {
...
pub export fn make_rand() c_uint {
    if (mk_state == uint) {
        mk_state = @as(c_uint, @bitCast(@as(c_int, @truncate(time(null) ^ @as(time_t, @bitCast(@as(c_long, make_pid()))))))) +% uint;
    }
...
pub export fn xmalloc(arg_size: usize) ?*anyopaque {
    var size = arg_size;
    var result: ?*anyopaque = malloc(if (size != 0) size else @as(usize, 1));
    if (result == null) {
        out_of_memory();
    }
    return result;
...
pub export fn xcalloc(arg_size: usize) ?*anyopaque {
    var size = arg_size;
    var result: ?*anyopaque = calloc(if (size != 0) size else @as(usize, 1), ulong);
    if (result == null) {
        out_of_memory();
    }
    return result;
...
pub export fn collapse_continuations(arg_line: [*c]u8) void {
    ...
    while (true) {
        ...
        in = q + @as(usize, @bitCast(@as(isize, 1)));
        if ((i & 1) != 0) {
    ...
    _ = memmove(@as(?*anyopaque, @ptrCast(out)), @as(?*const anyopaque, @ptrCast(in)), strlen(in) +% ulong);
}
pub export fn lindex(arg_s: [*c]const u8, arg_limit: [*c]const u8, arg_c: c_int) [*c]u8 {
    ...
    while (s < limit) if (@as(c_int, @bitCast(@as(c_uint, (blk: {
        ...
        ref.* += 1;
        break :blk tmp;
    }).*))) == c) return @as([*c]u8, @ptrCast(@volatileCast(@constCast(s - @as(usize, @bitCast(@as(isize, 1)))))));
    return null;
}
    ...
    if (!(tmpdir.static != null)) {
        ...
        var found: c_uint = 0;
        {
            ...
            while (tp.* != null) : (tp += 1) if (((blk: {
                ...
                if (r < 0) {
                    c_error_fn_replaced_for_reserved_keyword(@as([*c]floc, @ptrFromInt(0)), (strlen(tp.*) +% strlen(tmpdir.static)) +% strlen(strerror(__errno_location().*)), gettext("%s value %s: %s"), tp.*, tmpdir.static, strerror(__errno_location().*));
                } else if (!((st.st_mode & @as(__mode_t, 61440)) == @as(__mode_t, 16384))) {
                    c_error_fn_replaced_for_reserved_keyword(@as([*c]floc, @ptrFromInt(0)), strlen(tp.*) +% strlen(tmpdir.static), gettext("%s value %s: not a directory"), tp.*, tmpdir.static);
                } else return tmpdir.static;
            };
        }
        tmpdir.static = "/tmp";
        if (found != 0) {
            c_error_fn_replaced_for_reserved_keyword(@as([*c]floc, @ptrFromInt(0)), strlen(tmpdir.static), gettext("using default temporary directory '%s'"), tmpdir.static);
        }
    }
...
pub export fn get_tmpfd(arg_name: [*c][*c]u8) c_int {
    ...
    if (fd < 0) {
        c_error_fn_replaced_for_reserved_keyword(@as([*c]floc, @ptrFromInt(0)), strlen(tmpnm) +% strlen(strerror(__errno_location().*)), gettext("cannot create temporary file %s: %s"), tmpnm, strerror(__errno_location().*));
    ...
    if (name != null) {
        ...
        if (r < 0) {
            c_error_fn_replaced_for_reserved_keyword(@as([*c]floc, @ptrFromInt(0)), strlen(tmpnm) +% strlen(strerror(__errno_location().*)), gettext("cannot unlink temporary file %s: %s"), tmpnm, strerror(__errno_location().*));
        }
        free(@as(?*anyopaque, @ptrCast(tmpnm)));
    }
    _ = umask(mask);
...
pub export fn get_tmpfile(arg_name: [*c][*c]u8) [*c]FILE {
    ...
    if (file_1 == @as([*c]FILE, @ptrCast(@alignCast(@as(?*anyopaque, @ptrFromInt(0)))))) {
        c_error_fn_replaced_for_reserved_keyword(@as([*c]floc, @ptrFromInt(0)), strlen(name.*) +% strlen(strerror(__errno_location().*)), gettext("fdopen: temporary file %s: %s"), name.*, strerror(__errno_location().*));
    }
    return file_1;
}
...
// /nix/store/1l5gb4773rbqjzv49wb4h9xlmaz2zs5h-zig-0.13.0/lib/zig/include/__stdarg_va_arg.h:17:9
pub const va_end = @compileError("unable to translate macro: undefined identifier `__builtin_va_end`");
// /nix/store/1l5gb4773rbqjzv49wb4h9xlmaz2zs5h-zig-0.13.0/lib/zig/include/__stdarg_va_arg.h:19:9
pub const va_arg = @compileError("unable to translate C expr: unexpected token 'an identifier'");
// /nix/store/1l5gb4773rbqjzv49wb4h9xlmaz2zs5h-zig-0.13.0/lib/zig/include/__stdarg_va_arg.h:20:9
pub const __va_copy = @compileError("unable to translate macro: undefined identifier `__builtin_va_copy`");
// /nix/store/1l5gb4773rbqjzv49wb4h9xlmaz2zs5h-zig-0.13.0/lib/zig/include/__stdarg___va_copy.h:11:9
pub const va_copy = @compileError("unable to translate macro: undefined identifier `__builtin_va_copy`");
// /nix/store/1l5gb4773rbqjzv49wb4h9xlmaz2zs5h-zig-0.13.0/lib/zig/include/__stdarg_va_copy.h:11:9
