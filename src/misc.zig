...
pub fn xmalloc(_t: type, size: c_ulonglong) []_t {
    // Make sure we don't allocate 0, for pre-ISO implementations.

    // バイトサイズから型に合わせて数を調整する
    const calc_size: usize = if (size == 0) 1 else @divExact(size, @sizeOf(_t));

    const result: []_t = std.heap.page_allocator.alloc(_t, calc_size) catch {
...
/// Retry short writes from EINTR.
/// Return LEN, or -1 on error.
pub fn writebuf(fd: c_int, buffer: []const u8, len: size_t) ssize_t {
    const io = @cImport({
        @cInclude("io.h");
    });
    const errno = @cImport({
    ...
    while (l != 0) {
        ...
        { // EINTRLOOP(r, write(fd, msg, l));
            ...
            r = io.write(fd, msg.ptr, @intCast(l));
            while (r == -1 and errno.errno == errno.EINTR) {
                r = io.write(fd, msg, l);
            }
        }
