...
pub export fn construct_vpath_list(arg_pattern: [*c]u8, arg_dirpath: [*c]u8) void {
    ...
    while ((@as(c_int, @bitCast(@as(c_uint, stopchar_map[@as(u8, @bitCast(dirpath.*))]))) & (2 | 64)) != 0) {
        dirpath += 1;
    }
    maxelem = 2;
    p = dirpath;
    ...
    while (@as(c_int, @bitCast(@as(c_uint, p.*))) != @as(c_int, '\x00')) {
        ...
        while ((@as(c_int, @bitCast(@as(c_uint, stopchar_map[@as(u8, @bitCast(p.*))]))) & (2 | 64)) != 0) {
            p += 1;
        }
    }
    if (elem > uint) {
        var path: [*c]struct_vpath = undefined;
        if (elem < (maxelem -% uint)) {
            vpath_1 = @as([*c][*c]const u8, @ptrCast(@alignCast(xrealloc(@as(?*anyopaque, @ptrCast(vpath_1)), @as(c_ulong, @bitCast(@as(c_ulong, elem +% uint))) *% @sizeOf([*c]const u8)))));
        }
        vpath_1[elem] = null;
        ...
        while (v != null) : (v = v.*.next) {
            ...
            _ = printf("vpath %s ", v.*.pattern);
            {
                ...
                while (v.*.searchpath[i] != null) : (i +%= 1) {
                    _ = printf("%s%c", v.*.searchpath[i], if (v.*.searchpath[i +% uint] == null) @as(c_int, '\n') else @as(c_int, ':'));
                }
            }
        }
    }
    ...
    if (general_vpath == null) {
        ...
        _ = fputs(gettext("\n# General ('VPATH' variable) search path:\n# "), stdout);
        {
            ...
            while (path[i] != null) : (i +%= 1) {
                _ = printf("%s%c", path[i], if (path[i +% uint] == null) @as(c_int, '\n') else @as(c_int, ':'));
            }
        }
    }
}
