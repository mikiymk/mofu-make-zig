...
pub export fn hash_init_directories() void {
    hash_init(&directories, ulong, &directory_hash_1, &directory_hash_2, &directory_hash_cmp);
    hash_init(&directory_contents, ulong, &directory_contents_hash_1, &directory_contents_hash_2, &directory_contents_hash_cmp);
}
...
pub const enum_cmd_state_34 = c_uint;
pub extern fn set_command_state(file: ?*struct_file, state: enum_cmd_state_34) void;
...
pub const enum_update_status_35 = c_uint;
pub extern fn update_goal_chain(goals: ?*struct_goaldep) enum_update_status_35;
...
pub fn directory_contents_hash_1(arg_key_0: ?*const anyopaque) callconv(.C) c_ulong {
    ...
    hash = @as(c_ulong, @bitCast(@as(c_ulong, (@as(c_uint, @bitCast(@as(c_uint, @truncate(key.*.dev)))) << 4) ^ @as(c_uint, @bitCast(@as(c_uint, @truncate(key.*.ino)))))));
    return hash;
}
...
pub fn find_directory(arg_name: [*c]const u8) callconv(.C) [*c]struct_directory {
    ...
    if (dc.*.counter != command_count) {
        ...
        if (dc.*.dirstream == @as(?*DIR, @ptrCast(@as(?*anyopaque, @ptrFromInt(0))))) {
            ...
            hash_init(&dc.*.dirfiles, ulong, &dirfile_hash_1, &dirfile_hash_2, &dirfile_hash_cmp);
            open_directories +%= 1;
            if (open_directories == uint) {
...
// src/debug.h:41:9
pub const _DIRENT_H = 1;
pub const d_fileno = @compileError("unable to translate macro: undefined identifier `d_ino`");
// /nix/store/1l5gb4773rbqjzv49wb4h9xlmaz2zs5h-zig-0.13.0/lib/zig/libc/include/generic-glibc/bits/dirent.h:47:9
pub const _DIRENT_HAVE_D_RECLEN = "";
pub const _DIRENT_HAVE_D_OFF = "";
pub const _DIRENT_HAVE_D_TYPE = "";
pub const _DIRENT_MATCHES_DIRENT64 = 1;
pub inline fn _D_EXACT_NAMLEN(d: anytype) @TypeOf(strlen(d.*.d_name)) {
    return strlen(d.*.d_name);
}
pub inline fn _D_ALLOC_NAMLEN(d: anytype) @TypeOf((@import("std").zig.c_translation.cast([*c]u8, d) + d.*.d_reclen) - (&d.*.d_name[@as(usize, 0)])) {
    return (@import("std").zig.c_translation.cast([*c]u8, d) + d.*.d_reclen) - (&d.*.d_name[@as(usize, 0)]);
}
pub inline fn IFTODT(mode: anytype) @TypeOf((mode & @import("std").zig.c_translation.promoteIntLiteral(c_int, 0o170000, .octal)) >> 12) {
    return (mode & @import("std").zig.c_translation.promoteIntLiteral(c_int, 0o170000, .octal)) >> 12;
}
pub inline fn DTTOIF(dirtype: anytype) @TypeOf(dirtype << 12) {
    return dirtype << 12;
}
...
pub inline fn REAL_DIR_ENTRY(dp: anytype) @TypeOf(dp.*.d_ino != 0) {
    return dp.*.d_ino != 0;
}
pub const FAKE_DIR_ENTRY = @compileError("unable to translate C expr: expected ')' instead got '='");
// src/dir.c:60:10
pub const MAX_OPEN_DIRECTORIES = 10;
pub const DIRECTORY_BUCKETS = 199;
pub inline fn MAKECMP(_x: anytype, _y: anytype) @TypeOf(if (_x < _y) -1 else if (_x == _y) 0 else 1) {
    return if (_x < _y) -1 else if (_x == _y) 0 else 1;
}
pub const DIRFILE_BUCKETS = 107;
